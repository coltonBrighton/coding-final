...
export default function Recipes() {
  ...
  const handleDelete = async (id: number) => {
    try {
      // Fetch all mealplan entries
      const mealPlanResponse = await fetch("http://localhost:3000/mealplan");
      const mealPlanData = await mealPlanResponse.json();
      console.log(mealPlanData);

      // Filter meal plan entries that match the recipeId
      const mealPlanEntriesToDelete = mealPlanData.filter(
        (entry: mealplan) => entry.recipeId === id
      );

      // Create an array of DELETE fetch promises
      const deleteMealPlanPromises = mealPlanEntriesToDelete.map(
        (entry: mealplan) =>
          fetch(`http://localhost:3000/mealplan/${entry.id}`, {
            method: "DELETE",
          })
      );

      // Execute all mealplan DELETE requests in parallel
      await Promise.all(deleteMealPlanPromises);

      // Delete the recipe itself
      await fetch(`http://localhost:3000/recipe/${id}`, { method: "DELETE" });

      // Update state to remove the deleted recipe from the UI
